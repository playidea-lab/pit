      ...
      <section className="page space-y-8">
        <div>
          ...
          <p className="muted mt-1 text-sm">
            ...
            팀에 보이는 것은 각자가 확인한 결정뿐입니다.
          </p>
        </div>

        {invites.length > 0 && (
          <div>
            <h2 className="mb-3 text-[15px] font-semibold text-ink">초대</h2>
            <div className="space-y-3">
              {invites.map(({ team, invited_by_login }) => (
                <div key={team.id} className="card flex flex-wrap items-center gap-3">
                  ...
                  <form action={leaveTeam}>
                    <input type="hidden" name="team_id" value={team.id} />
                    <button className="btn btn-ghost h-8 px-3">거절</button>
                  </form>
                </div>
              ))}
            </div>
