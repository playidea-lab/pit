...
판정기는 비용이 들지 않는 kNN — 오프라인 시험(G7)에서 확신도가 쓸모 있었다(확신 ≥ 0.6 → 정확도 0.89).
...
class TwinService:
    ...
    async def ask(self, caller: Caller, login: str, proposal: str, situation: str = "") -> dict[str, object]:
        ...
        prediction, confidence = self._judge(pool, query)
        ...
        return _answer(login, departed, None if abstained else prediction, confidence, abstained, evidence)

    def _judge(self, pool: list[StoredDecision], query: str) -> tuple[str | None, float]:
...
def _note(login: str, departed: bool, abstained: bool) -> str:
    ...
    if abstained:
        return f"{login}의 기록으로는 판단할 수 없어 {login}에게 질문을 남겼습니다. 답하면 그의 결정으로 기록됩니다."
